...
//        String[] excludePath = { "/emp/", "/swagger-ui/**", "/v3/**", "/swagger-resources/**", "/webjars/**", "/api-docs/**", "/emp/login" };
...
//        return Arrays.stream(excludePath).anyMatch(pattern -> path.matches(pattern.replace("**", ".*")));
		...
		// 필터가 적용될 경로
		String[] includePath = { "/emp/me" };
		String path = request.getRequestURI();

		// 경로가 매칭되는지 확인
		return Arrays.stream(includePath).noneMatch(pattern -> path.matches(pattern.replace("**", ".*")));
	}
